...
const cish::ASTBase& CishPass::handle(const StoreInst& store) {
  ...
  // If the destination starts with a '&', get rid of it. This happens because
  // the address is calculated using a getelementptr which adds the '&' but
  // the result of the getelementptr is then cast to the correct type. If
  // casts are being ignored, the '&' gets "propagated" when it would otherwise
  // get associated with the cast
